...
def image_list():
    try:
        return make_response(jsonify(image_service.retrieve_images()), 200)
...
def build_image():
    try:
        new_image = image_service.create_image(request.args)
        status_code = image_service.identify_status_code(new_image)
...
def delete_image():
    try:
        return make_response(jsonify(image_service.delete_image(request.args['tags'])), 202)
    except docker.errors.APIError as e:
        return make_response(jsonify({"error":"Server error", "details": str(e)}), 500)