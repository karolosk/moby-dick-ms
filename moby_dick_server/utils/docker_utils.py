from services.docker_service import docker_client, docker_low_level_client
import json
...
def calculate_cpu_percent(client):
    ...
    cpu_percent = 0.0
    cpu_delta = float(client["cpu_stats"]["cpu_usage"]["total_usage"]) - \
                float(client["precpu_stats"]["cpu_usage"]["total_usage"])
    system_delta = float(client["cpu_stats"]["system_cpu_usage"]) - \
                   float(client["precpu_stats"]["system_cpu_usage"])
    if system_delta > 0.0:
